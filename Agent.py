...
class Agent:
    def __init__(self, position, color, castlingRights, enPassantOpportunity):
        self.position = position
        self.color = color
        self.game = ChessSim(position, self.color, castlingRights, enPassantOpportunity)
    ...
    def evaluate(self):
        ...
        if self.game.gameEnded:
            if self.color == "w":
                if self.game.gameResult == 1:
                    score = float('+inf')
                elif self.game.gameResult == -1:
                    score = float('-inf')
                else:
                    score = 0
            else:
        ...
        for piece in self.game.position:
            pieceDiff += self.game.PIECE_ID_TO_VALUE_TRANSLATION[piece]
        ...
        back2RanksDiff = 0
        whiteBack2Ranks = self.game.position[:16]
        blackBack2Ranks = self.game.position[-16:]

        for piece in whiteBack2Ranks:
            # dont count the king and dont count opposite colored pieces
            if self.game.PIECE_ID_TO_VALUE_TRANSLATION[piece] > 0 and self.game.PIECE_ID_TO_VALUE_TRANSLATION[piece] != 9999:
                back2RanksDiff += self.game.PIECE_ID_TO_VALUE_TRANSLATION[piece]
        ...
        centerSquares = [self.game.position[27], self.game.position[28], self.game.position[35], self.game.position[36]]
        ...
        print(np.reshape(self.game.position, (8, 8)))
    ...
    def playBestMove(self):
        legalMoves = self.game.generateAllLegalMoves()

        selectedMove = self.selectRandomMove(legalMoves)


        return selectedMove
    
    def selectRandomMove(self, legalMoves):
        selectedMove = ""
        promotion = False
        for location in range(len(legalMoves)):
            if len(legalMoves[location]) != 0:
                ...
                if self.color == "w" and self.position[location] == 1 and selectedMove[3] == "8":
                    promotion = True
                elif self.color == "b" and self.position[location] == 7 and selectedMove[3] == "1":
                    promotion = True
                break
        if promotion:
